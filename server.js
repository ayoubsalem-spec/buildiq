  ...
  if (req.method === 'POST' && req.url === '/api/analyze') {
    ...
    req.on('end', async () => {
      try {
        ...
        const { imageBase64, width, height, mode, knownRoomName } = JSON.parse(body);
        ...
        } else if (knownRoomName) {
          // PASS 2 of the room-click flow -- this is a tight, high-resolution
          // crop of just the ONE room already identified in pass 1. The room
          // name isn't in question anymore; the only job here is precise,
          // trustworthy corner-by-corner trace instructions from a much more
          // detailed view than the wide first-pass image allowed.
          prompt = 'You are BuildIQ, an expert AI construction estimator. This image is a tight, zoomed-in crop showing ONLY the "' + knownRoomName + '" room and its immediate walls (' + width + 'x' + height + ' pixels) -- the room has already been identified, do not second-guess the name.\n\nLook carefully at exactly where this room\'s own walls are in THIS zoomed view. Give precise, GPS-style step-by-step corner-tracing instructions using only what is visible in this crop. Do not describe corners or walls that belong to a neighboring room -- if a wall in view separates this room from another, that wall IS this room\'s boundary, trace along it, do not cross it.\n\nReturn ONLY valid JSON in this exact format:\n{\n  "roomName": "' + knownRoomName + '",\n  "dimensions": "dimensions if visible in this crop",\n  "steps": [\n    {"stepNumber": 1, "action": "Click SW corner - START HERE", "detail": "Click the bottom-left inside corner", "direction": "START HERE"},\n    {"stepNumber": 2, "action": "Click NW corner", "detail": "Move UP along the west wall and click top-left corner", "direction": "GO NORTH"},\n    {"stepNumber": 3, "action": "Click NE corner", "detail": "Move RIGHT along the north wall and click top-right corner", "direction": "GO EAST"},\n    {"stepNumber": 4, "action": "Click SE corner", "detail": "Move DOWN along the east wall and click bottom-right corner", "direction": "GO SOUTH"},\n    {"stepNumber": 5, "action": "Double-click to close", "detail": "Double-click near your starting point to close the shape", "direction": "CLOSE SHAPE"}\n  ],\n  "doorsToSubtract": [],\n  "windowsToSubtract": []\n}';
        } else {
